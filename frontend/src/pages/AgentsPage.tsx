// src/pages/AgentOnlyDashboard.tsx
import { useState, useEffect, useCallback } from 'react'
import { useNavigate } from 'react-router-dom'
import { Clock, Ticket as TicketIcon, CheckCircle, LogOut, Users } from 'lucide-react'
...
export function AgentOnlyDashboard() {
  ...
  const handleClaim = async (ticket: Ticket) => {
    ...
    try {
      ...
      setClaimError('This ticket was just claimed by another agent.')
      await loadAll() // refresh so the stale ticket disappears from the inbox immediately
    } finally {
      setClaimingId(null)
    }
  }
  ...
  return (
    <div className="flex min-h-full flex-col bg-background">
      <header className="flex h-16 shrink-0 items-center justify-between border-b border-border bg-gradient-to-r from-accent to-accent-dark px-4 md:px-6">
        <div>
          ...
          <div className="flex items-center gap-1.5">
            <Users className="h-3.5 w-3.5 text-white/80" strokeWidth={2} />
            <span className="font-ui text-xs text-white/80">Agent{team ? ` — ${team}` : ''}</span>
          </div>
        </div>
      ...
      <main className="mx-auto w-full max-w-6xl flex-1 px-4 py-8 md:px-6">
        {claimError && (
          <div className="mb-4 rounded-lg border border-amber-300 bg-amber-50 px-4 py-2.5 font-ui text-sm text-amber-800">
            {claimError}
          </div>
        )}

        {/* KPI cards — agent-relevant only, no org-wide counts */}
        <div className="mb-6 grid grid-cols-3 gap-3">
          ...
          <div className="rounded-xl border-l-4 border-accent bg-surface p-4">
            <div className="mb-2 flex items-center gap-2">
              ...
              <span className="font-ui text-xs text-text-hint">My Active</span>
            </div>
          ...
          {(['inbox', 'active', 'resolved'] as Tab[]).map((t) => (
            <button
              ...
              className={`flex flex-1 items-center justify-center gap-2 rounded-lg px-4 py-2.5 font-ui text-sm font-medium transition-all ${
                tab === t ? 'bg-accent text-white' : 'text-text-secondary hover:bg-surface-muted'
              }`}
            >
              {t === 'inbox' ? 'Inbox' : t === 'active' ? 'My Active' : 'Resolved'}
